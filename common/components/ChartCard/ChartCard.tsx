'use client'
import {ChartCards, ChartField} from "@/common/types/types";
...
const ChartCard: React.FC<createCardProps> = ({
    ...
    const sumValueMonthly = monthlyData?.fields.reduce((acc, cur) => acc + cur.value, 0);

    const createLegendBar = useCallback(({value, sumValue, key, color}: legendBarProps) => {
        const flexCalc = value && sumValue ? value / sumValue * 100 : 0;
        return (
            <span style={{minWidth: "25px", flex: flexCalc, backgroundColor: color}} key={key}/>
        )
    }, [])

    const createLegendLabels = useCallback(({value, sumValue, key, icon}: LegendLabelProps) => {
    ...
    const createChart = useCallback(({sumValue, chartData}: ChartProps) => {

        const sortChartData = chartData.fields.sort((a, b) => b.value - a.value);

        const conicGradientArray: { counter: number, colorArray: string[] } = sortChartData.reduce((acc, el) => {
            acc.colorArray.push(`${el.color} ${acc.counter / sumValue * 100 + 0.2}%`)
            acc.colorArray.push(`${el.color} ${(acc.counter + el.value) / sumValue * 100}%`)
            acc.counter = acc.counter + el.value
            return acc
        }, {counter: 0, colorArray: []} as { counter: number, colorArray: string[] })

        const conicGradientString = `conic-gradient(${conicGradientArray.colorArray.toString()})`

        return (
            <>
                <div className={"Chart"} style={{backgroundImage: conicGradientString}}/>
                <div className={"ChartLegend"}>
                    <div className={"ChartLegendLabels"}>
                        {sortChartData.map((el: ChartField) => createLegendLabels({
                            sumValue: sumValue,
                            value: el.value,
                            key: el.fieldName,
                            icon: el.icon
                    ...
                    <div className={"ChartLegendBar"}>
                        {sortChartData.map((el: ChartField) => createLegendBar({
                            sumValue: sumValue,
                            value: el.value,
                            key: el.fieldName,
                            color: el.color
    ...
    const createField = useCallback((data: ChartField) => {
        return (
            <div className={"Field"} key={`${data.fieldName}(${data.paramName})`}>
                ...
                <span style={{color: data.color}} className={"ColoredText"}>{data.value}</span>
                {data.additionText ? <span>{`(${data.additionValue} ${data.additionText})`}</span> : null}
            </div>
        )
    }, [])
                ...
                isSwitchable && setIsMonthlyFunc ?
                    <section className={"SwitcherContainer"}>
                        <div className={"ButtonWrapper"}>
                            <label>
                                <input type={"radio"} onClick={() => setIsMonthlyFunc(false)}
                                       name={`${cardHeader}-time-selector`} value={`${cardHeader}-all-time`}
                                       defaultChecked={true}/>
                                <span>{"За всё время"}</span>
                            </label>
                            <label>
                                <input type={"radio"} onClick={() => setIsMonthlyFunc(true)}
                                       name={`${cardHeader}-time-selector`} value={`${cardHeader}-monthly`}
